...
\paragraph{}
Formally, given a dataset $A$ and $B$, $A$ and $B$ are said to be \textit{adjacent} if there is a single record $r$ that is in one but not the other. For adjacent datasets $A$ and $B$, a query $Q$ has $\epsilon$-differential privacy if for any adjacent datasets $A$ and $B$, and any subset $C$ of possible outcomes $Range(Q)$, 

$Pr[Q(A) in C] \leq exp(\epsilon) \times Pr[M(B) in C]$
...
$f(x) + L$ where L is $Laplace(0, \sigma)$, $\sigma \geq \frac{\delta f}{\epsilon}$, and $\delta f = max(||f(x) - f(x')||_1)$.

The PDF for the Laplace Distribution is defined as:

$\frac{1}{2b} \times exp(-\frac{|x - \mu|}{b})$

where $b$ is a scaling factor. The Manhattan norm is defined as: $||x|_1 = \frac{1}{n} \sum_1^n |x_1|$.
